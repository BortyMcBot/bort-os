...
function createFixPRs(findings) {
  ...
  for (const [file, fileFindings] of byFile) {
    ...
    const bodyPath = path.join(fs.mkdtempSync(path.join(os.tmpdir(), `pr-body-${slug}-`)), 'body.md')
    ...
    try {
      ...
      const fixPrompt = [
        ...
        `Only edit this file: ${file}`,
        'Do not modify any other file or create new files.',
      ...
      if (porcelain) {
        ...
        if (unexpected.length > 0) {
          ...
          throw new Error(`fixer touched files outside target: ${unexpectedPaths.join(', ')}`)
        }
      }
