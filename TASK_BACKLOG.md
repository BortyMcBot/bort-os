...
| T-002 | Restore persistent X API access (fix 401s) and harden token lifecycle so daily post + digest jobs stay authenticated | P1 | queued | M | Monday | Re-auth X OAuth, verify token scopes, run dry-run + post + digest checks, then add automatic refresh + failure circuit breaker | Done when `x_auth --dry-run` returns 200, daily post returns 201, digest avoids 401, and jobs auto-recover without manual re-auth |
| T-003 | Build website-maintenance automation for personal site repo (post-Claude-Code baseline): skills/jobs/tasks for upkeep and PR-driven workflow | P2 | queued | L | this month | After Claude Code scaffolds baseline in VSCode, capture repo conventions and implement maintenance automation (issue triage, update checks, branch+PR flow, reporting) | Done when recurring maintenance jobs run, PRs are opened automatically for scoped changes, and Bryan can review/approve/comment without manual commit workflow |
